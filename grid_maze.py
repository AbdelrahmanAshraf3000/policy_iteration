...
class GridMazeEnv(gym.Env):
    ...
    def step(self, action):
        ...
        perpendicular_actions = {
            0: [2, 3],  # For Up: Left and Right are perpendicular
            1: [2, 3],  # For Down: Left and Right are perpendicular
            2: [0, 1],  # For Left: Up and Down are perpendicular
            3: [0, 1]   # For Right: Up and Down are perpendicular
        }
        
        # Choose actual action based on probabilities
        prob = random.random()
        if prob < 0.7:  # 70% chance of intended action
            actual_action = action
        else:  # 15% chance each of perpendicular actions
        ...
        if actual_action == 0:  # Up -> y + 1
            y = min(y + 1, self.grid_size - 1)
        elif actual_action == 1:  # Down -> y - 1
            y = max(y - 1, 0)
        elif actual_action == 2:  # Left -> x - 1
            x = max(x - 1, 0)
        elif actual_action == 3:  # Right -> x + 1
